...
impl LearningSession {
    ...
    pub fn recognition_options(&self) -> Option<(Vec<String>, usize)> {
        ...
        Some(rotated_options(
            self.lesson.new_words.iter().map(|word| word.meaning.clone()).collect(),
            current,
            current + 1,
        ))
    }
    ...
    pub fn listening_options(&self) -> Option<(Vec<String>, usize)> {
        let current = self.current_mastery_index()?;
        Some(rotated_options(
            self.lesson.new_words.iter().map(|word| word.text.clone()).collect(),
            current,
            current + 2,
        ))
    }
    ...
    pub fn sentence_options(&self) -> Option<(Vec<String>, usize)> {
        let current = self.current_mastery_index()?;
        Some(rotated_options(
            self.lesson.sentences.iter().map(|item| item.meaning.clone()).collect(),
            current,
            current + 1,
        ))
    }
    ...
    fn advance_phase_after_mastery(&mut self) {
        match self.phase {
            Phase::Recognition => self.start_phase(Phase::Listening, self.lesson.new_words.len()),
            Phase::Listening => self.start_phase(Phase::Sentences, self.lesson.sentences.len()),
            Phase::Sentences => {
...
fn rotated_options(mut options: Vec<String>, correct_source_index: usize, rotation: usize) -> (Vec<String>, usize) {
    if options.is_empty() {
        return (options, 0);
    }
    let len = options.len();
